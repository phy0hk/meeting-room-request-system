...
function CheckCreateBookingBody(body) {
    if (!body.roomId)
    ...
    if (body.startTime == body.endTime)
        throw new BadRequestError("meeting start time and end time should not be the same");
    return body;
}
function CheckUpdateBookingBody(body) {
    if (!body.bookingId) {
    ...
    if (body.startTime && body.endTime) {
        if (new Date(body.startTime).getTime() ===
            new Date(body.endTime).getTime()) {
            throw new BadRequestError("Start and end time should not be the same");
        }
    }
    return body;
}
...
async function CreateBooking(bookingInfo) {
    if (bookingInfo.startTime > bookingInfo.endTime) {
        throw new BadRequestError("Start time should not be greater the end time");
    }
    ...
    if (bookingsCheck.length > 0) {
        throw new BadRequestError("Time slot overlaps with an existing booking");
    }
...
export async function UpdateBooking(bookingId, updateData) {
    ...
    if (startTime > endTime) {
        throw new BadRequestError("Start time should not be greater than end time");
    }
    ...
    if (bookingsCheck.length > 0) {
        throw new BadRequestError("New time slot overlaps with an existing booking");
    }
